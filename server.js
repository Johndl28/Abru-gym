...
const uri = process.env.MONGODB_URI || 'mongodb+srv://abru_admin:abrupassword@calorietracker.qhiq6tw.mongodb.net/?retryWrites=true&w=majority&appName=calorieTracker';
...
app.get('/api/routines', async (req, res, next) => {
  try {
    const db = client.db('calorieTracker');
    const routines = await db.collection('abru_routines').find().toArray();
...
app.post('/api/routines', async (req, res, next) => {
  try {
    ...
    await db.collection('abru_routines').updateOne(
      { date: routine.date },
      { $set: routine },
      { upsert: true }
    );
...
app.delete('/api/routines/:date', async (req, res, next) => {
  try {
    ...
    const result = await db.collection('abru_routines').deleteOne({ date });
...
app.get('/api/meals', async (req, res, next) => {
  try {
    const db = client.db('calorieTracker');
    const meals = await db.collection('abru_calories').find().toArray();
...
app.post('/api/meals', async (req, res, next) => {
  try {
    ...
    await db.collection('abru_calories').insertOne(meal);
...
app.delete('/api/meals/:date/:meal', async (req, res, next) => {
  try {
    ...
    const result = await db.collection('abru_calories').deleteOne({ date, meal });
    if (result.deletedCount === 0) {
